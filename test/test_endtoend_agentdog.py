...
'''
End to end tests for AgentDog with several examples
'''
...
class TestAgentDog(unittest.TestCase):
    ...
    def test_from_local_to_galaxy(self, name, json_path, xml_path):
        ...
        bioagent = main.json_to_bioagent(json)
        tmp_file = 'tmp_test_xml.xml'
        main.write_xml(bioagent,tmp_file)
    ...
    @parameterized.expand(get_test_files_xml())
    def test_from_bioagents_to_galaxy(self, name, json_path, xml_path):
        # Open json to be the content of the requests_mock
        json_answer = main.json_from_file(json_path)
        with requests_mock.mock() as m:
            m.get('https://bio.agents/api/agent/' + name + '/version/1.0',\
                  json=json_answer)
            json = main.json_from_bioagents(name, '1.0')
            bioagent = main.json_to_bioagent(json)
            tmp_file = 'tmp_test_xml.xml'
            main.write_xml(bioagent,tmp_file)
            tmp_file_list = glob("tmp_*.xml")
    ...
    def test_from_local_to_cwl(self, name, json_path, cwl_path):
        ...
        main.write_cwl(bioagent,tmp_file)
    ...
    @parameterized.expand(get_test_files_cwl())
    def test_from_bioagents_to_cwl(self, name, json_path, cwl_path):
        # Open json to be the content of the requests_mock
        json_answer = main.json_from_file(json_path)
        with requests_mock.mock() as m:
            m.get('https://bio.agents/api/agent/' + name + '/version/1.0',\
