...
# Class and Objects
from agentdog import main, bioagent_model
from agentdog.annotate import galaxy, cwl, edam_to_galaxy
...
class TestBioagent(unittest.TestCase):

    def setUp(self):
        self.bioagent = bioagent_model.Bioagent('name', 'an_id', 'a_version', 'a description '+\
                                       'with spaces.', 'http://urltohomepage.com')

    def test_init(self):
        self.assertEqual(self.bioagent.name, 'name')
        self.assertEqual(self.bioagent.agent_id, 'an_id')
        self.assertEqual(self.bioagent.version, 'a_version')
        self.assertEqual(self.bioagent.description, 'a description with spaces.')
        self.assertEqual(self.bioagent.homepage, 'http://urltohomepage.com')
        self.assertListEqual(self.bioagent.functions, [])
        self.assertListEqual(self.bioagent.topics, [])
        #self.assertIsNone(self.bioagent.informations)

    def test_set_informations(self):
        agent_credits = [{'comment':'a_comment', 'email':'an_email',\
        ...
        self.bioagent.set_informations(agent_credits, contacts, pubs, docs, language, link, download)
        # Check credits params
        self.assertEqual(self.bioagent.informations.agent_credits[0].comment, 'a_comment')
        self.assertEqual(self.bioagent.informations.agent_credits[0].email, 'an_email')
        self.assertEqual(self.bioagent.informations.agent_credits[0].grid_id, 'a_grid_id')
        self.assertEqual(self.bioagent.informations.agent_credits[0].name, 'a_name')
        self.assertEqual(self.bioagent.informations.agent_credits[0].type_entity, 'a_type_entity')
        self.assertEqual(self.bioagent.informations.agent_credits[0].type_role, 'a_type_role')
        self.assertEqual(self.bioagent.informations.agent_credits[0].url, 'a_url')
        self.assertEqual(self.bioagent.informations.agent_credits[0].orcid_id, 'an_orcid_id')
        # Check contacts params
        self.assertEqual(self.bioagent.informations.contacts[0].email, 'an_email')
        self.assertEqual(self.bioagent.informations.contacts[0].name, 'a_name')
        # Check publications params
        self.assertEqual(self.bioagent.informations.publications[0].doi, 'a_doi')
        self.assertEqual(self.bioagent.informations.publications[0].pmid, 'a_pm_id')
        self.assertEqual(self.bioagent.informations.publications[0].pmcid, 'a_pmc_id')
        self.assertEqual(self.bioagent.informations.publications[0].type, 'a_type')
        # Check documentations params
        self.assertEqual(self.bioagent.informations.documentations[0].url, 'an_url')
        self.assertEqual(self.bioagent.informations.documentations[0].type, 'a_type')
        self.assertEqual(self.bioagent.informations.documentations[0].comment, 'a_comment')
        # Check language params
        self.assertEqual(self.bioagent.informations.language[0], 'Python')
        # Check links
        self.assertEqual(self.bioagent.informations.links[0].url, 'a_url')
        self.assertEqual(self.bioagent.informations.links[0].type, 'Repository')
        self.assertEqual(self.bioagent.informations.links[0].comment, 'a comment')
        self.assertEqual(self.bioagent.informations.links[1].url, 'a_url')
        self.assertEqual(self.bioagent.informations.links[1].type, 'Source code')
        self.assertEqual(self.bioagent.informations.links[1].comment, 'a comment')
    ...
    def test_add_functions(self):
        ...
        self.bioagent.add_functions(function)
        # Check function
        self.assertEqual(self.bioagent.functions[0].operations[0].uri, EDAM_OPE['uri'])
        self.assertEqual(self.bioagent.functions[0].operations[0].term, EDAM_OPE['term'])
        # Check inputs
        self.assertEqual(self.bioagent.functions[0].inputs[0].data_type.uri,\
                         EDAM_DATA['uri'])
        self.assertEqual(self.bioagent.functions[0].inputs[0].data_type.term, \
                         EDAM_DATA['term'])
        self.assertEqual(self.bioagent.functions[0].inputs[0].formats[0].uri,\
                         EDAM_FORMAT['uri'])
        self.assertEqual(self.bioagent.functions[0].inputs[0].formats[0].term, \
                         EDAM_FORMAT['term'])
        self.assertIsNone(self.bioagent.functions[0].inputs[0].description)
        # Check outputs
        self.assertEqual(self.bioagent.functions[0].outputs[0].data_type.uri, \
                         EDAM_DATA['uri'])
        self.assertEqual(self.bioagent.functions[0].outputs[0].data_type.term, \
                         EDAM_DATA['term'])
        self.assertEqual(self.bioagent.functions[0].outputs[0].formats[0].uri, \
                         EDAM_FORMAT['uri'])
        self.assertEqual(self.bioagent.functions[0].outputs[0].formats[0].term, \
                         EDAM_FORMAT['term'])
        self.assertIsNone(self.bioagent.functions[0].outputs[0].description)

    def test_add_topics(self):
        topics = [EDAM_TOPIC]
        self.bioagent.add_topics(topics)
        self.assertEqual(self.bioagent.topics[0].uri, EDAM_TOPIC['uri'])
        self.assertEqual(self.bioagent.topics[0].term, EDAM_TOPIC['term'])


class TestInformations(unittest.TestCase):

    def test_init(self):
        info = bioagent_model.Informations()
        self.assertListEqual(info.publications, [])
        self.assertListEqual(info.documentations, [])
        self.assertListEqual(info.contacts, [])
        self.assertListEqual(info.agent_credits, [])
...
class TestCredit(unittest.TestCase):
    ...
    def test_init(self):
        ...
        credit = bioagent_model.Credit(dict_credit)
...
class TestPublication(unittest.TestCase):
    ...
    def test_init(self):
        ...
        publication = bioagent_model.Publication(dict_publi)
...
class TestDocumentation(unittest.TestCase):
    ...
    def test_init(self):
        ...
        documentation = bioagent_model.Documentation(dict_doc)
...
class TestContact(unittest.TestCase):
    ...
    def test_init(self):
        ...
        contact = bioagent_model.Contact(dict_contact)
...
class TestFunction(unittest.TestCase):
    ...
    def setUp(self):
        self.function = bioagent_model.Function([EDAM_TOPIC])
...
class TestData(unittest.TestCase):
    ...
    def test_init(self):
        ...
        data = bioagent_model.Data(data_type, formats, description)
...
class TestEdam(unittest.TestCase):
    ...
    def setUp(self):
        self.edam = bioagent_model.Edam(EDAM_TOPIC)
...
class TestImportJson(unittest.TestCase):
    ...
    #def test_json_from_bioagent(self):


class TestMainFunctions(unittest.TestCase):

    def test_json_to_bioagent(self):
        json_path = os.path.dirname(__file__) + '/MacSyFinder.json'
        j = main.json_from_file(json_path)
        biot = main.json_to_bioagent(j)
        # Check 3/5 arguments of bioagent object
        self.assertEqual(biot.name, 'MacSyFinder')
        self.assertEqual(biot.agent_id, 'MacSyFinder')
        ...
        self.assertEqual(biot.informations.agent_credits[0].name, 'Institut Pasteur')
...
class TestGalaxyAgentGen(unittest.TestCase):

    def setUp(self):
        # Create a bioagent
        self.bioagent = bioagent_model.Bioagent('a_name', 'an_id', 'a_version', 'a_description.',\
                                     'a_homepage')
        self.genxml = galaxy.GalaxyAgentGen(self.bioagent)

    def test_init(self):
        # Test counters
        self.assertEqual(self.genxml.input_ct, 0)
        self.assertEqual(self.genxml.output_ct, 0)
        # Copy agent to make it easier to read
        agent = self.genxml.agent
        # Test simple values of the agent
        self.assertEqual(agent.version_command, "COMMAND --version")
        # Test <agent> of the future XML
        self.assertEqual(agent.root.attrib['id'], 'an_id')
        self.assertEqual(agent.root.attrib['name'], 'a_name')
        self.assertEqual(agent.root.attrib['version'], 'a_version')
        # Test <description> of the future XML
        self.assertEqual(agent.root.find('description').text, 'a_description.')

    def test_init_existing(self):
        genxml = galaxy.GalaxyAgentGen(self.bioagent,
                                    existing_agent=os.path.dirname(__file__) +
                                                  '/test_write_xml.xml')
        self.assertEqual(genxml.agent.version_command, 'COMMAND --version')
        self.assertEqual(genxml.agent.command, 'COMMAND')
        self.assertEqual(genxml.agent.root.attrib['id'], 'an_id')
        self.assertEqual(genxml.agent.root.attrib['name'], 'a_name')
        self.assertEqual(genxml.agent.root.attrib['version'], 'a_version')

    def test_add_edam_topic(self):
        # Create a Topic object
        topic = bioagent_model.Topic(EDAM_TOPIC)
        self.genxml.add_edam_topic(topic)
        # Test
        self.assertEqual(self.genxml.agent.edam_topics.children[0].node.text, 'topic_0091')

    def test_add_edam_operation(self):
        # Create a Operation object (Warning: EDAM_TOPIC is a topic)
        operation = bioagent_model.Operation(EDAM_OPE)
        self.genxml.add_edam_operation(operation)
        # Test
        self.assertEqual(self.genxml.agent.edam_operations.children[0].node.text, 'operation_2429')

    def test_add_input_file(self):
        # Create a Input object (Warning both Type and Format will be a topic)
        an_input = bioagent_model.Input(EDAM_DATA, [EDAM_FORMAT])
        self.genxml.add_input_file(an_input)
        # Copy object to test (easier to read)
        input_attrib = self.genxml.agent.inputs.children[0].node.attrib
    ...
    def test_add_output_file(self):
        ...
        output = bioagent_model.Output(EDAM_DATA, [EDAM_FORMAT])
        self.genxml.add_output_file(output)
        # Copy object to test
        output_attrib = self.genxml.agent.outputs.children[0].node.attrib
    ...
    def test_add_citation(self):
        ...
        publication = bioagent_model.Publication(dict_pub)
        self.genxml.add_citation(publication)
        # Test
        self.assertEqual(self.genxml.agent.citations.children[0].node.text, 'doi:123')
        self.assertEqual(self.genxml.agent.citations.children[0].node.attrib['type'], 'doi')
...
class TestCwlAgentGen(unittest.TestCase):

    def setUp(self):
        # Create a bioagent
        self.bioagent = bioagent_model.Bioagent('a_name', 'an_id', 'a_version', 'a_description.',
                                     'a_homepage')
        self.gencwl = cwl.CwlAgentGen(self.bioagent)
    ...
    def test_init(self):
        ...
        # Copy agent to make it easier to read
        agent = self.gencwl.agent
        # Test simple values of the agent
        self.assertEqual(agent.id, "an_id")
        self.assertEqual(agent.label, "a_description.")
        self.assertEqual(agent.baseCommand, "COMMAND")
        self.assertListEqual(agent.inputs, [])
        self.assertListEqual(agent.outputs, [])

    def test_init_existing(self):
        gencwl = cwl.CwlAgentGen(self.bioagent,
                                existing_agent=os.path.dirname(__file__) +
                                              '/test_write_cwl.cwl')
        self.assertEqual(gencwl.agent.id, 'an_id')
        self.assertEqual(gencwl.agent.label, 'a_description.')
        self.assertEqual(gencwl.agent.cwlVersion, 'v1.0')

    def test_add_input_file(self):
        # Create a Input object (Warning both Type and Format will be a topic)
        input = bioagent_model.Input(EDAM_DATA, [EDAM_FORMAT])
        self.gencwl.add_input_file(input)
        # Copy object to test (easier to read)
        input_attrib = self.gencwl.agent.inputs[0]
    ...
    def test_add_output_file(self):
        ...
        # Copy object to test
        output_attrib = self.gencwl.agent.outputs[0]
