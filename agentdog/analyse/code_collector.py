...
class CodeCollector(object):
    """
    Class to download source code from a https://bio.agents entry
    """
    ...
    def _get_from_repository(self, url):
        ...
        if "github.com" in url:
            ...
            LOGGER.warn("AgentDog only deals with Github repository for the moment...")
    ...
    def get_source(self):
        """
        Retrieve source code of the agent using links provided in https://bio.agents
        """
        source_code = None
        links = self.bioagent.informations.links
        for link in links:
            link_type = link.type.lower().translate(str.maketrans(' ', '_'))
            try:
                source_code = getattr(self, '_get_from_{}'.format(link_type))(link.url)
            except AttributeError:
                LOGGER.warn(link_type + ' link type is not processed yet by AgentDog.')
